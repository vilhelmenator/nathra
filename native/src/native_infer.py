...
from ast_nodes import TAG_SUBSCRIPT, TAG_IF_EXP, TAG_LIST_COMP, TAG_JOINED_STR
...
def native_infer_call_type(s: ptr[CompilerState], node: ptr[AstNode]) -> str:
    ...
    if func.tag == TAG_ATTRIBUTE:
        ...
        if strmap_has(addr_of(s.structs), base):
            method_name: str = str_concat(base + "_", pa.attr)
            ret3: str = strmap_get(addr_of(s.func_ret_types), method_name)
            if ret3 is not None:
                return ret3
        # String methods
        if obj_type == "NrStr*":
            if pa.attr == "len" or pa.attr == "find":
                return str_new("int64_t")
            if pa.attr == "eq" or pa.attr == "contains":
                return str_new("int")
            return str_new("NrStr*")
